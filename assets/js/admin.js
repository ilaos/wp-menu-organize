    ...
    jQuery(document).ready(function($) {
        ...
        window.wmoCheckWordPressCSS = function() {
            ...
            if (container) {
                ...
                console.log('Elements overlapping with menu container:', overlappingElements.length);
                if (overlappingElements.length > 0) {
                    overlappingElements.slice(0, 5).forEach(function(item, index) {
                        console.log('  Overlapping element', index, ':', item.tagName, item.className);
                    });
                }
            }
        };

        // Add function to force enable drag interactions
        window.wmoForceEnableDrag = function() {
            console.log('=== WMO Force Enable Drag ===');
            
            // Force enable pointer events on all menu elements
            $('.menu-items-list, .menu-items-list *, #wmo-sortable-menu, #wmo-sortable-menu *').css({
                'pointer-events': 'auto !important',
                'cursor': 'grab !important'
            });
            
            // Force enable on handles specifically
            $('.menu-item-handle').css({
                'pointer-events': 'auto !important',
                'cursor': 'grab !important',
            ...
            console.log('Forced drag interactions enabled - try dragging now');
        };

        var menuColors = wmo_ajax.menuColors || {};

        // Show success message if menu order was saved
        if (sessionStorage.getItem('menuOrderSaved') === 'true') {
            $('#wmo-save-order').after('<div id="wmo-success-msg" class="notice notice-success"><p>Menu order updated successfully!</p></div>');
            setTimeout(function() {
                $('#wmo-success-msg').fadeOut('slow', function() { $(this).remove(); });
            }, 3000);
            sessionStorage.removeItem('menuOrderSaved');
        }

        // Function to save menu order via AJAX
        function saveMenuOrder() {
            var newOrder = $sortableMenu.sortable('toArray', { attribute: 'data-slug' });
            console.log('WMO: Saving menu order:', newOrder);

            if (newOrder.length === 0) {
                console.error('WMO: No menu items found to save');
                return;
            }
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    console.log('WMO: Sending AJAX request to save menu order');
                    $('#wmo-save-order').prop('disabled', true).text('Saving...');
                },
                success: function(response) {
                    console.log('WMO: AJAX Response:', response);
                    if (response.success) {
                        console.log('WMO: New order received:', response.data.order);
                        sessionStorage.setItem('menuOrderSaved', 'true');
                        $('#wmo-save-order').prop('disabled', false).text('Save Menu Order');
                        location.reload();
                    } else {
                        console.error('WMO: AJAX error response:', response.data);
                        alert('Error saving menu order: ' + (response.data || 'Unknown error'));
                        $('#wmo-save-order').prop('disabled', false).text('Save Menu Order');
                    }
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    console.error('WMO: AJAX error:', textStatus, errorThrown);
                    console.log('WMO: Response Text:', jqXHR.responseText);
                    var errorMessage = 'An error occurred. Please check the console and try again.';
                    if (jqXHR.status === 0) {
        ...
        $('#wmo-save-order').on('click', function() {
            console.log('WMO: Save button clicked');
            saveMenuOrder();
        });
        ...
        if ($.fn.wpColorPicker) {
            $('.wmo-color-field').wpColorPicker({
                change: function(event, ui) {
                    ...
                    console.log('WMO: Color picker changed:', slug, color, isSubmenu);
        ...
        function wmoApplyColor(slug, color, isSubmenu, isPreview = false) {
            console.log('WMO: Applying color:', slug, color, isSubmenu, isPreview ? '(preview)' : '');
            var menuItems = [];

            if (slug === 'all-in-one-wp-migration') {
                menuItems = [document.querySelector('#toplevel_page_ai1wm_export > a')];
            } else if (isSubmenu) {
                menuItems = Array.from(document.querySelectorAll('#adminmenu .wp-submenu li a')).filter(item =>
            ...
            menuItems.forEach(function(menuItem) {
                if (menuItem) {
                    console.log('WMO: Applying color to:', menuItem);
