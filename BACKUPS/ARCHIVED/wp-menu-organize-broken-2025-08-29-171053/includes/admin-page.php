...
class WP_Menu_Organize
{
    ...
    public function render_settings_page()
    {
        ...
        // Template called - including admin-settings-page.php template
    ...
    {
        // Debug: Log the hook suffix to see what it actually is
        // Check for the correct hook names - the main page is 'toplevel_page_wp-menu-organize-settings'
        ...
        // Remove ALL duplicate jQuery UI loading - use ONLY WordPress built-in
        wp_enqueue_script('jquery');
        wp_enqueue_script('jquery-ui-sortable');
        wp_enqueue_style('wp-color-picker');
        wp_enqueue_script('wp-color-picker');
        
        // DO NOT load CDN fallback - it's causing conflicts
        // Remove this entirely:
        // wp_enqueue_script('jquery-ui-cdn-fallback', ...)
        
        // Enqueue our custom CSS file using secure asset URLs
        wp_enqueue_style(
            'wp-menu-organize-style',
        ...
        // Load your script with proper dependencies
        wp_enqueue_script(
            'wmo-admin-script',
        ...
        wp_enqueue_script(
            ...
            plugin_dir_url(dirname(__FILE__)) . 'assets/js/color-picker.js',
            array('jquery', 'wp-color-picker', 'wmo-admin-script'),
            '1.0.0',
            true
        );
        ...
        wp_enqueue_script(
            ...
            plugin_dir_url(dirname(__FILE__)) . 'assets/js/templates.js',
            array('jquery', 'wmo-color-picker-script'),
            '1.0.0',
            true
        );
                ...
                wp_localize_script('wmo-admin-script', 'wmo_ajax', array(
                    ...
                    'menuColors' => wmo_get_settings('colors'),
                    'hook_suffix' => $hook_suffix // Add hook suffix for debugging
                ));
                
                // Also localize to color picker script
                wp_localize_script('wmo-color-picker-script', 'wmo_ajax', array(
                    'ajax_url' => admin_url('admin-ajax.php'),
                    'nonce' => wp_create_nonce('wmo_ajax_nonce'),
                    'menuColors' => wmo_get_settings('colors'),
                    'hook_suffix' => $hook_suffix // Add hook suffix for debugging
        ...
        // Add secure CSS output hook
        add_action('admin_enqueue_scripts', 'wmo_output_custom_css', 20);
        
        // Apply saved theme preference
        add_action('admin_head', array($this, 'apply_theme_preference'));
    }
...
// Apply custom icons globally
add_action('admin_head', 'wmo_apply_custom_icons');
    
function wmo_apply_custom_icons() {
    ...
    if (!empty($icons)) {
        // Decode emoji values before passing to JavaScript with improved encoding handling
        $decoded_icons = array();
        foreach ($icons as $menu_id => $icon_data) {
            if ($icon_data['type'] === 'emoji') {
                // Ensure proper UTF-8 encoding and decode the JSON-encoded emoji
                $emoji_value = $icon_data['value'];
                $emoji_value = mb_convert_encoding($emoji_value, 'UTF-8', 'UTF-8');
                $decoded_value = json_decode('"' . $emoji_value . '"');
                if ($decoded_value) {
                    $icon_data['value'] = $decoded_value;
                } else {
                    // Fallback: try direct UTF-8 decoding
                    $icon_data['value'] = mb_convert_encoding($emoji_value, 'UTF-8', 'UTF-8');
                }
            }
            $decoded_icons[$menu_id] = $icon_data;
        }
        
        // Pass decoded icons to JavaScript
        echo '<script>var wmo_saved_icons = ' . json_encode($decoded_icons) . ';</script>';
        
        // Add debug to see what's being sent to JavaScript
        // Removed debug logging for production
        
        // Add CSS to hide original icons
        echo '<style>';
        foreach ($decoded_icons as $menu_id => $icon_data) {
            // Hide the original dashicon
            echo '#adminmenu #' . esc_attr($menu_id) . ' .wp-menu-image::before { content: none !important; }';
            echo '#adminmenu #' . esc_attr($menu_id) . ' .wp-menu-image { content: none !important; }';
        }
        echo '</style>';
    } else {
        // No icons to apply
