...
function wmo_save_menu_order()
{
    ...
    if (!is_array($new_order)) {
        wmo_ajax_error('invalid_data_format', 'validation');
        return;
    }
    ...
    foreach ($new_order as $slug) {
        if (!is_string($slug) || empty($slug)) {
            wmo_ajax_error('invalid_menu_id', 'validation');
            return;
        }
    }
    ...
    if (!empty($new_order)) {
        ...
        wmo_ajax_error('missing_required_field', 'validation');
    }
}
...
function wmo_export_configuration()
{
    if (!current_user_can('manage_options')) {
        wmo_ajax_error('insufficient_permissions', 'permission');
        return;
    }

    if (!check_ajax_referer('wmo_ajax_nonce', 'nonce', false)) {
        wmo_ajax_error('security_check_failed', 'security');
    }
    ...
    . ', Typography: ' . ($export_typography ? 'yes' : 'no') . ', Badges: ' . ($export_badges ? 'yes' : 'no') . ', Theme: ' . ($export_theme ? 'yes' : 'no'));
...
function wmo_preview_import()
{
    ...
    if (empty($preview['summary'])) {
        wmo_ajax_error('import_failed', 'validation');
        return;
    }
...
function wmo_export_settings_ajax() {
    ...
    if ($export_data !== false) {
        ...
        wmo_ajax_error('export_failed', 'database');
    }
}
