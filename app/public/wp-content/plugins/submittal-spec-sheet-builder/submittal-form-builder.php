 ...
 * @copyright 2025 Webstuffguy Labs
...
// License logs admin page
require_once plugin_dir_path(__FILE__) . 'Includes/admin/license-logs.php';
...
final class SFB_Plugin {
  ...
  function ensure_tables() {
    ...
    // 5) License Operation Log (Compliance/Audit Trail)
    $license_log = $wpdb->prefix . 'sfb_license_log';
    $sql_license_log = "
    ...
    ";
    dbDelta($sql_license_log);
    if (!empty($wpdb->last_error)) {
      error_log('SFB ensure_tables LICENSE_LOG error: ' . $wpdb->last_error);
    }
  ...
  function api_get_license() {
    // Use the proper license data function
    ...
    return [
      ...
      'license' => [
        ...
        'activations_remaining' => $license_data['activations_remaining'] ?? null,
        'last_check' => $license_data['last_check'] ?? 0,
        'error' => $license_data['error'] ?? '',
      ],
      'pro_active' => sfb_is_pro_active(),
      'agency_active' => sfb_is_agency_license(),
    ];
  }
  ...
  function api_save_license($req) {
    ...
    $result = sfb_activate_license($key, $email);

    // Handle activation errors
    if (is_wp_error($result)) {
      return $result; // Return WP_Error directly (REST API handles it)
    }

    // Success - get updated license data
    $license_data = sfb_get_license_data();
  ...
  private function default_settings(){
    return [
      ...
      // NEW: Branding themes + watermark + signature
      'theme' => 'engineering',  // 'engineering' | 'architectural' | 'corporate'
      'watermark' => '',          // optional text watermark; empty disables
      'signature_block_default' => false, // Pro: include approval signature block by default
      // Draft settings
        ...
        <div class="sfb-branding-form">
          <form method="post" action="options.php" id="sfb-branding-form">
            ...
            <div class="sfb-card">
              ...
              <?php
              // Check if Pro license is active for theme and watermark features
              ...
              <div class="sfb-field-group">
                <label class="sfb-field-label" for="sfb-pdf-theme">
                  ...
                  <?php else: ?>
                    <span class="sfb-free-badge">FREE: Engineering Only</span>
                  <?php endif; ?>
                </label>
                <p class="sfb-field-hint">
              ...
              <!-- Signature Block (Pro Feature) -->
              <div class="sfb-field-group">
                <label class="sfb-checkbox-wrapper">
                  <input type="checkbox"
                         id="sfb-signature-block-default"
                         name="<?php echo esc_attr($this->option_key()); ?>[signature_block_default]"
                         value="1"
                         <?php checked(!empty($options['signature_block_default'])); ?>
                         <?php echo !$is_pro ? 'disabled' : ''; ?>>
                  <span class="sfb-checkbox-label">
                    <strong>
  ...
  function sanitize_settings($input) {
    ...
    // Signature Block - Pro only
    $sanitized['signature_block_default'] = $is_pro && !empty($input['signature_block_default']);
    ...
    try {
      ...
      foreach ($full_products as $product) {
        ...
        $formatted_products[] = [
          ...
          'meta' => $specs, // For template compatibility
          'note' => $note,
  ...
  */
  ...
  function api_generate_frontend_pdf_rest($p) {
    try {
      ...
      // Agency feature: use default preset if enabled
      if (sfb_is_agency_license() && get_option('sfb_brand_use_default_on_pdf', false)) {
        $default_preset = SFB_Branding::get_default_preset();
        if ($default_preset && !empty($default_preset['data'])) {
          $brand = wp_parse_args($default_preset['data'], $brand);
        }
      }
      ...
      $meta = [
        'project'        => sfb_text($project_name),
        'contractor'     => '',
        'submittal'      => '',
        'include_leed'   => false,
        'include_cover'  => !empty($brand['cover_default']),
        ...
        'email_to'       => '',
        'white_label'    => false,
        'approve_block'  => !empty($brand['signature_block_default']), // Use branding setting
        'approved_by'    => '',
        'approved_title' => '',
        'approved_date'  => '',
      ];
      ...
      // --- Generate HTML using template-based system ---
...
<?php
      ...
      if (!empty($meta['include_cover'])) {
        if (file_exists($template_dir . 'cover.html.php')) {
          $items = $formatted_products; // Make $items available to template
          include $template_dir . 'cover.html.php';
        }
      }

      // 2) Product sheets
      foreach ($grouped_products as $category_name => $products) {
        foreach ($products as $product) {
...
<?php
  ...
  private function stream_html_as_pdf_or_html_fallback($html, $filename = 'document.pdf') {
    ...
    $dompdf_autoload = plugin_dir_path(__FILE__) . 'lib/dompdf/autoload.inc.php';
...
<?php
  ...
  /** Rate limiting check */
  /**
   * Check rate limit using transient-based request counting
   *
   * @param string $action Action identifier (e.g., 'pdf_generate', 'draft_create')
   * @param int $max_requests Maximum requests allowed
   * @param int $time_window Time window in seconds (default 3600 = 1 hour)
   * @return array Rate limit result with 'allowed', 'count', 'limit', 'retry_after'
   */
  private function check_rate_limit($action, $max_requests = 5, $time_window = 3600) {
    // Get client IP (handle proxies like lead capture does)
    $ip = $this->get_client_ip();

    // Create unique transient key for this IP and action
    $transient_key = 'sfb_rl_' . $action . '_' . md5($ip);

    // Get current request count
    $request_data = get_transient($transient_key);

    if ($request_data === false) {
      // No requests yet - initialize
      $request_data = [
        'count' => 1,
        'first_request' => time(),
      ];
    ...
    // Increment count
    $request_data['count']++;
    $allowed = $request_data['count'] <= $max_requests;

    // Calculate time remaining in window
    $elapsed = time() - $request_data['first_request'];
    $retry_after = max(0, $time_window - $elapsed);

    // Update transient
    set_transient($transient_key, $request_data, $retry_after);
   ...
   * Same implementation as lead capture for consistency
   */
