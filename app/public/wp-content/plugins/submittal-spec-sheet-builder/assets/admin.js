(function () {
  ...
  function BrandingBox(){
    ...
    function load(){
      setLoading(true);
      // Load both settings and Pro status
      Promise.all([
        wp.apiFetch({ path:'/sfb/v1/settings' }),
        wp.apiFetch({ path:'/sfb/v1/status' })
      ])
        .then(([settingsRes, statusRes]) => {
          if(settingsRes?.ok){ setS(settingsRes.settings); }
          if(statusRes?.ok){ setProActive(statusRes.pro_active); }
        })
    ...
    return h('div', {className:'sfb-settings'},
      ...
      h('div', {className:'sfb-grid'},
        ...
        // PDF Footer Text - Pro Only
        proActive && h('label', null, 'PDF Footer Text (Pro)'),
        proActive && h('input', {type:'text', value:s.footer_text, onChange:e=>setS({...s, footer_text:e.target.value}), style:{width:'100%'}, placeholder:'Leave empty to remove branding'}),
        proActive && h('p', {style:{fontSize:'12px', color:'#6b7280', margin:'4px 0 0 0'}}, 'Customize the footer text on PDFs, or leave empty to remove branding entirely.')
      ),
