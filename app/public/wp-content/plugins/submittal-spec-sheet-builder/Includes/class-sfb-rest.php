...
final class SFB_Rest {
  ...
  public static function save_settings($req) {
    ...
    // Theme - enforce Pro gate for premium themes (architectural, corporate)
    $requested_theme = in_array($in['theme'] ?? '', ['engineering', 'architectural', 'corporate']) ? $in['theme'] : 'engineering';
    $theme = (!$is_pro && in_array($requested_theme, ['architectural', 'corporate'], true)) ? 'engineering' : $requested_theme;

    // Watermark - Pro only
    $watermark = $is_pro ? sanitize_text_field($in['watermark'] ?? '') : '';
