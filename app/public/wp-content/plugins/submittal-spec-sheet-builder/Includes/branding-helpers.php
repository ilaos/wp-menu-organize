...
function sfb_get_brand_settings() {
  ...
  if ($brand !== null && isset($brand['company']) && isset($brand['visual'])) {
    // Merge with defaults to ensure all fields exist (including white_label)
    $brand = array_replace_recursive($defaults, $brand);
  ...
  if (!empty($old)) {
    $migrated = [
      ...
      'white_label' => $old['white_label'] ?? $defaults['white_label'],
      '_meta' => [
        'version'    => '1.1.0',
        'updated_at' => current_time('mysql'),
