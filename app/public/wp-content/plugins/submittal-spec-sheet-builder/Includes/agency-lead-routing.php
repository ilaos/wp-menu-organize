...
class SFB_Agency_Lead_Routing {
   ...
   * @return array Result with 'valid' (bool) and 'error' (string) keys
   */
  private static function validate_webhook_url_ssrf(string $url): array {
    // Validate HTTPS requirement
    if (strpos($url, 'https://') !== 0) {
      return [
        'valid' => false,
        'error' => __('Webhook URL must use HTTPS', 'submittal-spec-sheet-builder')
      ];
    }
    ...
    if (!$parsed || !isset($parsed['host'])) {
      return [
        'valid' => false,
        'error' => __('Invalid webhook URL format', 'submittal-spec-sheet-builder')
      ];
    }
    ...
    if (in_array($host, $localhost_patterns, true)) {
      return [
        'valid' => false,
        'error' => __('Webhook URL cannot target localhost', 'submittal-spec-sheet-builder')
      ];
    }

    // Resolve hostname to IP address
    // Use gethostbyname for IPv4 (built-in PHP function)
    $ip = gethostbyname($host);

    // If gethostbyname fails, it returns the hostname unchanged
    if ($ip === $host && !filter_var($host, FILTER_VALIDATE_IP)) {
      // Hostname didn't resolve and isn't an IP - could be invalid or DNS issue
      // We'll allow it but log a warning
      if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log("SFB Webhook: Could not resolve hostname: $host");
      }
      return ['valid' => true, 'error' => ''];
    }

    // Validate the resolved IP address
    $ip_validation = self::validate_ip_address($ip);
    if (!$ip_validation['valid']) {
      return $ip_validation;
    }

    // Check if host itself is an IPv6 address
    if (filter_var($host, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6)) {
      $ipv6_validation = self::validate_ipv6_address($host);
      if (!$ipv6_validation['valid']) {
   ...
   * @return array Result with 'valid' (bool) and 'error' (string) keys
   */
  private static function validate_ip_address(string $ip): array {
    // Validate IP format
    if (!filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
      return ['valid' => true, 'error' => '']; // Not IPv4, skip validation
    }
    ...
    if ($ip_long === false) {
      return [
        'valid' => false,
        'error' => __('Invalid IP address format', 'submittal-spec-sheet-builder')
      ];
    }
    ...
    $blocked_ranges = [
      ...
      ['start' => ip2long('10.0.0.0'), 'end' => ip2long('10.255.255.255'), 'name' => 'private network (10.0.0.0/8)'],

      // Private: 172.16.0.0/12 (RFC 1918)
      ['start' => ip2long('172.16.0.0'), 'end' => ip2long('172.31.255.255'), 'name' => 'private network (172.16.0.0/12)'],

      // Private: 192.168.0.0/16 (RFC 1918)
      ['start' => ip2long('192.168.0.0'), 'end' => ip2long('192.168.255.255'), 'name' => 'private network (192.168.0.0/16)'],

      // Link-local: 169.254.0.0/16
      ['start' => ip2long('169.254.0.0'), 'end' => ip2long('169.254.255.255'), 'name' => 'link-local (169.254.0.0/16)'],

      // Multicast: 224.0.0.0/4
      ['start' => ip2long('224.0.0.0'), 'end' => ip2long('239.255.255.255'), 'name' => 'multicast (224.0.0.0/4)'],

      // Reserved: 240.0.0.0/4
      ['start' => ip2long('240.0.0.0'), 'end' => ip2long('255.255.255.255'), 'name' => 'reserved (240.0.0.0/4)'],

      // Broadcast: 255.255.255.255/32
      ['start' => ip2long('255.255.255.255'), 'end' => ip2long('255.255.255.255'), 'name' => 'broadcast'],

      // This network: 0.0.0.0/8
      ['start' => ip2long('0.0.0.0'), 'end' => ip2long('0.255.255.255'), 'name' => 'this network (0.0.0.0/8)'],
    ];

    // Check if IP falls within any blocked range
   ...
   * @return array Result with 'valid' (bool) and 'error' (string) keys
   */
  private static function validate_ipv6_address(string $ip): array {
    // Validate IPv6 format
    if (!filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6)) {
      return ['valid' => true, 'error' => '']; // Not IPv6, skip validation
    }

    // Normalize IPv6 address
    $ip_normalized = strtolower($ip);

    // Check for loopback (::1)
    if ($ip_normalized === '::1' || $ip_normalized === '0000:0000:0000:0000:0000:0000:0000:0001') {
      return [
        'valid' => false,
        'error' => __('Webhook URL cannot target IPv6 loopback (::1)', 'submittal-spec-sheet-builder')
      ];
    }

    // Check for link-local (fe80::/10)
    if (strpos($ip_normalized, 'fe80:') === 0 || strpos($ip_normalized, 'fe8') === 0 || strpos($ip_normalized, 'fe9') === 0 ||
        strpos($ip_normalized, 'fea') === 0 || strpos($ip_normalized, 'feb') === 0) {
      return [
        'valid' => false,
        'error' => __('Webhook URL cannot target IPv6 link-local addresses (fe80::/10)', 'submittal-spec-sheet-builder')
      ];
    }

    // Check for unique local (fc00::/7)
    if (strpos($ip_normalized, 'fc') === 0 || strpos($ip_normalized, 'fd') === 0) {
      return [
        'valid' => false,
        'error' => __('Webhook URL cannot target IPv6 private addresses (fc00::/7)', 'submittal-spec-sheet-builder')
      ];
    }

    // Check for deprecated site-local (fec0::/10) - similar to RFC 1918
    if (strpos($ip_normalized, 'fec') === 0 || strpos($ip_normalized, 'fed') === 0 ||
        strpos($ip_normalized, 'fee') === 0 || strpos($ip_normalized, 'fef') === 0) {
      return [
        'valid' => false,
        'error' => __('Webhook URL cannot target IPv6 site-local addresses (fec0::/10)', 'submittal-spec-sheet-builder')
      ];
    }
  ...
  public static function send_webhook(array $context, array $rule, int $attempt = 1): array {
    ...
    $validation = self::validate_webhook_url_ssrf($webhook_url);
    if (!$validation['valid']) {
      if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log("SFB Webhook: Blocked SSRF attempt - {$validation['error']} - URL: $webhook_url");
      }
      return [
        'status' => 'failed',
        'code' => 0,
