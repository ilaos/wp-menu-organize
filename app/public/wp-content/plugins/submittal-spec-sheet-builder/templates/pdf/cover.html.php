...
<?php endif; ?>
<div style="text-align:center; padding:80px 40px; color:<?php echo  esc_attr($accent); ?>;">
  <?php $logo_url = sfb_text($brand['logo_url'] ?? ''); ?>
  <?php if ($logo_url !== ''): ?>
    <img src="<?php echo  esc_url($logo_url); ?>" style="max-height:120px; margin-bottom:30px;">
  <?php endif; ?>
  <?php $company_name = sfb_text($brand['company_name'] ?? ''); ?>
  <h1 style="font-size:32px; font-weight:700; margin:0 0 10px;"><?php echo  esc_html($company_name !== '' ? $company_name : 'Submittal Package'); ?></h1>
  ...
  <?php if ($project !== '' || $contractor !== '' || $submittal !== ''): ?>
    <div style="margin-top:40px; font-size:16px; color:#333;">
      <?php if ($project !== ''): ?>
        <p style="margin:8px 0;"><strong>Project:</strong> <?php echo  esc_html($project); ?></p>
      <?php endif; ?>
      <?php if ($contractor !== ''): ?>
        <p style="margin:8px 0;"><strong>Contractor:</strong> <?php echo  esc_html($contractor); ?></p>
      <?php endif; ?>
      <?php if ($submittal !== ''): ?>
        <p style="margin:8px 0;"><strong>Submittal #:</strong> <?php echo  esc_html($submittal); ?></p>
      <?php endif; ?>
    </div>
  <?php endif; ?>

  <div style="margin-top:40px; font-size:14px; color:#666;">
    <p style="margin:4px 0;"><strong>Date:</strong> <?php echo  esc_html(sfb_text($meta['date'] ?? '') ?: date('F j, Y')); ?></p>
    <p style="margin:4px 0;"><strong>Total Products:</strong> <?php echo  count($items); ?></p>
  </div>

  <?php if (!empty($meta['include_leed'])): ?>
    <div style="margin-top:30px; padding:10px 20px; background:#f0f9ff; border:2px solid <?php echo  esc_attr($bar); ?>; border-radius:8px; display:inline-block;">
      <strong style="color:<?php echo  esc_attr($bar); ?>;">✓ LEED Information Included</strong>
    </div>
  <?php endif; ?>
