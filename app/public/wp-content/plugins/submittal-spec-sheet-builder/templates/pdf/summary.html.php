...
<div style="max-width:800px; margin:0 auto; padding:28px 34px;">
  <h2 style="color:<?php echo  esc_attr($bar); ?>; font-size:24px; font-weight:700; margin:0 0 8px; border-bottom:2px solid <?php echo  esc_attr($bar); ?>; padding-bottom:10px;">
    Summary
  </h2>
  <p style="font-size:13px; color:#6b7280; margin:0 0 20px; padding:0;">
    <?php echo  count($items); ?> item<?php echo  count($items) === 1 ? '' : 's'; ?> total • <?php echo  count($groups); ?> categor<?php echo  count($groups) === 1 ? 'y' : 'ies'; ?>
  </p>

  <?php foreach ($groups as $cat => $rows): ?>
    <h3 style="margin:16px 0 8px 0; font-size:16px; font-weight:600; color:#111827;">
      <?php echo  esc_html($cat); ?> <span style="color:#6b7280; font-weight:400;">(<?php echo  count($rows); ?>)</span>
    </h3>
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse; font-size:12px; margin-bottom:16px;">
      <thead>
        <tr>
          <th style="text-align:left; padding:6px 8px; background:<?php echo  esc_attr($bar); ?>; color:#fff; width:42%; border:1px solid <?php echo  esc_attr($bar); ?>; font-weight:600;">
            Item
          </th>
          <th style="text-align:left; padding:6px 8px; background:<?php echo  esc_attr($bar); ?>; color:#fff; border:1px solid <?php echo  esc_attr($bar); ?>; font-weight:600;">
            Key Specifications
          </th>
        </tr>
      </thead>
        ...
        <?php foreach ($rows as $it): ?>
          ...
          <tr>
            <td style="padding:6px 8px; border:1px solid #e5e7eb; background:#fff; vertical-align:top;">
              <strong style="color:#111827;"><?php echo  esc_html($item_title); ?></strong><br>
              <span style="color:#6b7280; font-size:11px;"><?php echo  esc_html(implode(' › ', $item_path)); ?></span>
            </td>
